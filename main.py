...
from utils import get_map_image, geocode, find_business
...
class MapApp(QMainWindow):
    ...
    def mousePressEvent(self, event):
        ...
        if not (map_pos.x() <= x <= map_pos.x() + 600 and
                map_pos.y() <= y <= map_pos.y() + 450):
            return
        rel_x, rel_y = x - map_pos.x(), y - map_pos.y()
        lon_step = 360 / (2 ** self.zoom * 256)
        lat_step = 180 / (2 ** self.zoom * 256)
        new_lon = self.lon + (rel_x - 300) * lon_step
        new_lat = self.lat - (rel_y - 225) * lat_step
        if event.button() == Qt.MouseButton.LeftButton:
            pos, address, postal = geocode(f"{new_lon},{new_lat}")
        elif event.button() == Qt.MouseButton.RightButton:
            pos, address, postal = find_business((new_lon, new_lat))
        else:
            return
        if pos:
            self.marker = pos
